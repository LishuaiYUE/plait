...
import { PlaitBaseTable, PlaitTable, PlaitTableBoard, PlaitTableCell, PlaitTableCellWithPoints, TableSymbols } from '../interfaces/table';
...
import { DefaultTableProperty } from '../constants/table';
...
export const createItems = (count: number) => new Array(count).fill('').map((_) => ({ id: idCreator() }));
...
export const createTableCells = (rows: { id: string; height?: number }[], columns: { id: string; width?: number }[]) => {
...
export const createDefaultTable = (points: [Point, Point]) => {
    const rows = createItems(3);
    const columns = createItems(3);
    const table: PlaitTable = {
        id: idCreator(),
        type: TableSymbols.table,
        points,
        rows,
        columns,
...
export const getDefaultTablePoints = (centerPoint: Point) => {
    return RectangleClient.getPoints(
        RectangleClient.getRectangleByCenterPoint(centerPoint, DefaultTableProperty.width, DefaultTableProperty.height)
    );
};
